...
class LinearModel(nn.Module):
    def __init__(self, n_feature=192, h_dim=3 * 32, n_output=200):
...
class ResNet(nn.Module):
    def __init__(self, in_channels, block_features, num_classes=200, headless=False):
        super().__init__()
...
class VIB(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_y = logits_y.reshape((B, 200))  # (B,   10)
...
def init_vib(args):
    if args.dataset == 'MNIST':
        ...
        decoder = LinearModel(n_feature=args.dimZ, n_output=3 * 128 * 128)
        lr = args.lr
...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, train_type):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        loss = args.beta * KLD_mean + H_p_q + BCE  #- args.mcr_rate * nuclear_norm #  + H_p_q + H_p_q - nuclear_norm  + H_p_q + H_p_q + nuclear_norm
        ...
        if step % len(dataset) % 10000 == 0:
            ...
            x_cpu = x.cpu().data
            x_cpu = x_cpu.clamp(0, 1)
            x_cpu = x_cpu.view(x_cpu.size(0), 3, args.image_size, args.image_size)
            ...
            # x_hat_cpu = x_hat_cpu.view(x_hat_cpu.size(0), 3, args.image_size, args.image_size)
...
def eva_vib(vib, dataloader_erase, args, name='test', epoch=999):
    ...
    for batch_idx, (x, y) in enumerate(dataloader_erase):
        x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 200)
        # x = x.view(x.size(0), -1)
        # print(x.shape)
        logits_z, logits_y, x_hat, mu, logvar = vib(x, mode='with_reconstruction')  # (B, C* h* w), (B, N, 200)
...
def eva_S_VIB(vib, dataloader_erase, args, name='test', epoch=999):
    ...
    for batch_idx, (x, y) in enumerate(dataloader_erase):
        ...
        # x = x.view(x.size(0), -1)
        # print(x.shape)
        logits_y, logits_z, x_hat, mu, logvar, KLD_mean  = vib(x, mode='S-VAE-C')  # (B, C* h* w), (B, N, 200)
...
def eva_vib_target(vib, dataloader_erase, args, name='test', epoch=999):
    ...
    for batch_idx, (x, y_t, y_ad) in enumerate(dataloader_erase):
        x, y_t, y_ad = x.to(args.device), y_t.to(args.device), y_ad.to(args.device)  # (B, C, H, W), (B, 200)
        # x = x.view(x.size(0), -1)
        # print(x.shape)
        logits_z, logits_y, x_hat, mu, logvar = vib(x, mode='with_reconstruction')  # (B, C* h* w), (B, N, 200)
...
def train_reconstructor(vib, train_loader, reconstruction_function, args):
    ...
    for epoch in range(epochs):
        ...
        for grad, img in train_loader:
            grad, img = grad.to(args.device), img.to(args.device)  # (B, C, H, W), (B, 200)
            grad = grad.view(grad.size(0), 1, 16, 16)
            # img = img.view(img.size(0), -1)  # Flatten the images
            output = reconstructor(grad)
            # output = output.view(output.size(0), 3, args.image_size, args.image_size)
...
def prepare_centroid_and_positive(vib, unlearning_loader, remaining_loader, args):
    ...
    with torch.no_grad():
        for step, (images, labels) in enumerate(remaining_loader):
            ...
            z, logits_y, x_hat, mu, logvar = vib(images, mode='with_reconstruction')  # (B, C* h* w), (B, N, 200)
...
def create_unlearning_with_topk_loader(vib, unlearning_loader, remaining_loader, args, top_k=5, shuffle=True):
    ...
    all_remaining_images = torch.cat(all_remaining_images_list, dim=0).to('cpu')  # shape [N, C, H, W]
    all_remaining_embeddings = torch.cat(all_remaining_embeddings, dim=0).to('cpu')  # [N, d]
    all_remaining_labels = torch.cat(all_remaining_labels, dim=0).to('cpu')          # [N]
    ...
    with torch.no_grad():
        for step, (images, labels) in enumerate(unlearning_loader):
            ...
            sim = torch.mm(z_unlearn.cpu(), all_remaining_embeddings.T)  # [B, N], dot product => cosine
...
def unlearning_with_topk(vib, unlearning_loader_with_center, top_k_nearest_loader, loss_fn, reconstruction_function, args):
    ...
    for epoch in range(args.unl_epochs):
        ...
        for x, center_z_new, y, positive_x in unlearning_loader_with_center:
            ...
            loss = 0.1  * (F.mse_loss(logits_z_p, center_z_new) - 1.01 * F.mse_loss(logits_z, center_z_new) + alpha )
...
class MultiViewTinyImageNet(Dataset):
    """
    Wraps a TinyImageNetDataset (or any image dataset returning PIL images)
    to produce `k` augmented views per sample, just like MultiViewMNIST.
    """
    def __init__(self, base_dataset, k,
                 aug_transform=None):   # e.g. ColorJitter + RandomCrop + ToTensor
        self.base_dataset = base_dataset
        self.k = k
        # self.base_transform = base_transform
        self.aug_transform = aug_transform
    ...
    def __getitem__(self, idx):
        # 1) fetch raw PIL image and its class id
        img, label = self.base_dataset[idx]

        # 2) deterministic transform once (resize / normalize, etc.)
        # if self.base_transform is not None:
        #     img = self.base_transform(img)

        # 3) draw k stochastic views
        if self.aug_transform is None:
            # if no aug specified, just copy the (maybe‑transformed) image k times
            views = [img for _ in range(self.k)]
        else:
            views = [self.aug_transform(img) for _ in range(self.k)]

        # 4) return the list of views and optionally the labels duplicated
        labels = [label] * self.k
        return views, labels
...
if args.dataset == 'MNIST':
    ...
    # deterministic step: resize to 64×64 & convert to tensor

    # train_dir = "/kaggle/input/tiny-imagenet/tiny-imagenet-200/train" # "/kaggle/input/tiny-imagenet/tiny-imagenet-200/tiny-imagenet-200/train" # "/home/weiqiwan/Data/data/immagenet/train"  #
    #
    # val_dir = "/home/weiqiwan/Data/data/tiny-imagenet-200/tiny-imagenet-200/val"
    # test_dir = "/kaggle/input/tiny-imagenet/tiny-imagenet-200/val" # "/home/weiqiwan/Data/data/immagenet/test"  #

    # Set local Data directory for TinyImageneet
    # data_dir = '/kaggle/input/tiny-imagenet/tiny-imagenet-200'
    data_dir = "/home/weiqiwan/Data/data/tiny-imagenet-200/tiny-imagenet-200"
...
acc_r = eva_vib(vib_unlearnined, remaining_loader, args, name='on the remaining training after unlearning', epoch=999)
