...
class VIB(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_z = add_rician_noise(logits_z, torch.tensor(args.SNR).cuda(), K=torch.tensor(
                2).cuda())  # add_rayleigh_noise(logits_z, torch.tensor(args.SNR).cuda()) #
...
def train_reconstructor(vib, train_loader, reconstruction_function, args):
    ...
    for epoch in range(epochs):
        ...
        print("cosine similarity:", sum(similarity_term) / len(similarity_term), "average loss:",
...
def evaluate_reconstructor(vib, reconstructor, train_loader, reconstruction_function, args):
    ...
    for epoch in range(1):
        ...
        print(f'Epoch [{epoch + 1}/{epochs}], Loss: {loss.item():.4f}')
        print("constructed learning cosine similarity:", sum(similarity_term) / len(similarity_term), "average loss:",
              sum(loss_list) / len(loss_list))
    return reconstructor
...
def connect_determined_alpha(model_a, x, positive_x, center_z_new, args):
    ...
    with torch.no_grad():
        ...
        alpha = 1.1 * F.mse_loss(logits_z, center_z_new) - F.mse_loss(logits_z_p, center_z_new)

    return alpha


def unlearning_with_topk(vib_unlearnined, unlearning_loader_with_center, top_k_nearest_loader, loss_fn, reconstruction_function,
                         args):
    optimizer = torch.optim.Adam(vib_unlearnined.parameters(), lr=args.lr)
    vib_unlearnined.train()
    for epoch in range(args.unl_epochs):
        epoch_loss = 0

        for x, center_z_new, y, positive_x in unlearning_loader_with_center:
            x, center_z_new, y, positive_x = x.to(args.device), center_z_new.to(args.device), y.to(
                args.device), positive_x.to(args.device)
            logits_z, logits_y, x_hat, mu, _ = vib_unlearnined(x, mode='with_reconstruction')
            logits_z_p, logits_y_p, _, mu_p, _ = vib_unlearnined(positive_x, mode='with_reconstruction')

            H_p_q = loss_fn(logits_y_p, y)
            alpha = connect_determined_alpha(copy.deepcopy(vib_unlearnined), x, positive_x, center_z_new, args)

            x_hat = x_hat.view(x_hat.size(0), -1)
            x = x.view(x.size(0), -1)
            BCE = reconstruction_function(x_hat, x)

            loss = 0.2 * (F.mse_loss(logits_z_p, center_z_new) - 1.1 * F.mse_loss(logits_z,
                                                                                   center_z_new) + 1* alpha)  # + H_p_q # with label
        ...
        print(
            f"Epoch {epoch + 1}/{args.unl_epochs}, Loss: {epoch_loss:.4f}, H_p_q:{H_p_q.item()}, BCE:{BCE.item()}, alpha:{alpha.item()}")
...
######
# a = 1
# os.chdir(os.path.dirname(os.path.abspath(__file__)))  # 改变工作目录到当前文件所在的目录
seed = 0
...
seed = 5               # any integer you like
random.seed(seed)            # sets the global RNG state
...
vib = torch.load('/kaggle/input/vib/pytorch/default/1/vib_model.pth', map_location=args.device)
vib.to(args.device)
...
#     vib.train()
#     vib = vib_train(train_loader, vib, loss_fn, reconstruction_function, args, epoch,
#                     train_type)  # dataloader_total, dataloader_w_o_twin
# # vib = torch.load('./vib_model.pth', map_location=args.device)
# vib_path = './vib_model.pth'
# torch.save(vib, vib_path)  # 保存模型
...
labeled_unlearn_set = CustomLabelDataset(unlearning_dataset,
                                         0)  # we set label to 0 as the unlearned should not in the training set
...
unlearning_with_new_center, top_k_nearest_loader = create_unlearning_with_topk_loader(vib, unlearning_loader,
                                                                                      remaining_loader, args,
                                                                                      top_k=5, shuffle=True)
...
vib_unlearnined = unlearning_with_topk(vib_unlearnined, unlearning_with_new_center, top_k_nearest_loader, loss_fn,
...
acc_r = eva_vib(vib_unlearnined, original_train_loader, args, name='on the remaining training after unlearning',
                epoch=999)
acc_ua = eva_vib(vib_unlearnined, unlearning_loader, args, name='on the unlearning data after unlearning',
                 epoch=999)
print("acc_ua:", 1 - acc_ua)
