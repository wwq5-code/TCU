...
@torch.no_grad()
def eva_vib_fp16(vib, dataloader, args, name='test', epoch=999):
    vib.eval()
    num_total, num_correct, MSE_total = 0, 0, 0.0

    # 若想全局都用 FP16，确保模型在 GPU 上：
    # vib.to(torch.float16)  # 一般不推荐这么做，因为 BatchNorm、某些 ops 可能不支持

    for x, y in dataloader:
        x = x.to(args.device, non_blocking=True)
        y = y[:, 20].to(args.device, non_blocking=True)

        # 开启混合精度上下文
        with autocast():
            logits_z, logits_y, x_hat, mu, logvar = vib(x, mode='with_reconstruction')
            # 重建损失：在 autocast 下，BCE 也会在 FP16 计算
            x_hat = x_hat.view(x_hat.size(0), -1)
            x_flat = x.view(x.size(0), -1)
            mse = reconstruction_function(x_hat, x_flat)
            MSE_total += mse.item()
    ...
    print(f'epoch {epoch}, {name} accuracy: {acc:.4f}, avg_mse: {avg_mse:.6f}')
    return acc
...
def get_membership_inf_model(original_train_set, test_set, vib, args):
    ...
    selected_trained_set, temp_remain = torch.utils.data.random_split(original_train_set, [8000, original_size - 8000])
    test_size = len(test_set)
    selected_test_set, temp_remain = torch.utils.data.random_split(test_set, [8000, test_size - 8000])
...
# 2. 冻结 approximator 和 decoder（只让 encoder 可训练）
for name, param in vib.named_parameters():
    if not name.startswith('encoder.'):
        param.requires_grad = False
# 验证
print("可训练参数：", [n for n, p in vib.named_parameters() if p.requires_grad])
vib.to(args.device)
...
eval_loader = DataLoader(
    original_train_set,
    batch_size=128,  # e.g. 128 or 256
    shuffle=False,
    num_workers=4,  # e.g. 4 or 8
    pin_memory=True,
    prefetch_factor=2
)
vib.eval()
...
acc_r = eva_vib_fp16(vib, eval_loader, args, name='on the remaining training before unlearning', epoch=999)
...
acc_r = eva_vib_fp16(vib_unlearnined, eval_loader, args, name='on the remaining training after unlearning', epoch=999)
