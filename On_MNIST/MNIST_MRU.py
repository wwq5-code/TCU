...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, train_type):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        logits_y, logits_z, x_hat, mu, logvar, KLD_mean  = model(x, mode='S-VAE-C')  # (B, C* h* w), (B, N, 10)
        ...
        loss = args.beta * KLD_mean + BCE  - nuclear_norm  # + H_p_q - nuclear_norm  + H_p_q + H_p_q + nuclear_norm
...
vib.eval()
acc = eva_S_VIB(vib, test_loader, args, name='on test dataset', epoch=999)
...
with torch.no_grad():
    for step, (images, labels) in enumerate(test_loader):
        ...
        logits_y, z, x_hat, mu, logvar, KLD_mean = vib(images, mode='S-VAE-C')  # (B, C* h* w), (B, N, 10)
...
all_embeddings = torch.cat(all_embeddings, dim=0).cpu().numpy()  # [N, d]
all_labels = torch.cat(all_labels, dim=0).cpu().numpy()          # [N]
...
emb_2d = tsne.fit_transform(all_embeddings2)

# Plot the 2D embeddings
plt.figure(figsize=(10,10))
scatter = plt.scatter(emb_2d[:,0], emb_2d[:,1], c=all_labels2, cmap='tab10', alpha=0.7, s=10)
...
# Put the model into evaluation mode and initialize placeholders
# vib.eval()
# all_embeddings = []
# all_labels = []
#
...
#         logits_y, z, x_hat, mu, logvar, KLD_mean = vib(images, mode='S-VAE-C')
#         z = F.normalize(z, p=2, dim=1)  # Normalize the embeddings
#         all_embeddings.append(z.cpu())
#         all_labels.append(labels)
#
# # Convert embeddings and labels into numpy arrays
# all_embeddings = torch.cat(all_embeddings, dim=0).cpu().numpy()  # Shape: [N, d]
# all_labels = torch.cat(all_labels, dim=0).cpu().numpy()  # Shape: [N]
#
# # Optionally limit the number of samples for better t-SNE visualization
# N = 5000
# all_embeddings2 = all_embeddings[:N]
# all_labels2 = all_labels[:N]
#
...
# emb_2d = tsne.fit_transform(all_embeddings2)
#
# # Normalize the embeddings to fit within a circle
# scaler = MinMaxScaler(feature_range=(-1, 1))
# emb_2d = scaler.fit_transform(emb_2d)
#
# # Map embeddings onto a circle
# theta = np.arctan2(emb_2d[:, 1], emb_2d[:, 0])
# emb_2d[:, 0] = np.cos(theta)
# emb_2d[:, 1] = np.sin(theta)
#
# # Plot the circular embeddings
# plt.figure(figsize=(8, 8))
# scatter = plt.scatter(
#     emb_2d[:, 0], emb_2d[:, 1], c=all_labels2, cmap='tab10', alpha=0.8, s=8, edgecolor='none'
# )
# plt.colorbar(scatter, ticks=range(10))
# plt.title("2D Circular Visualization of Learned Representations")
# plt.xlabel("Dimension 1")
# plt.ylabel("Dimension 2")
# plt.axis("off")  # Turn off the axes for a cleaner look
# plt.gca().set_aspect('equal', adjustable='datalim')  # Enforce equal aspect ratio
#
# # Save and show the plot
# plt.savefig("circular_learned_representations_fixed.png", dpi=300, bbox_inches='tight')  # Save with high resolution
# plt.show()
