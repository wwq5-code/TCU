...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, train_type):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        loss = args.beta * KLD_mean + BCE + H_p_q  #  + H_p_q + H_p_q - nuclear_norm  + H_p_q + H_p_q + nuclear_norm
...
def vib_train_with_MCR(dataset, model, loss_fn, reconstruction_function, args, epoch, train_type):
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        loss = args.beta * KLD_mean + BCE + H_p_q - nuclear_norm #  + H_p_q + H_p_q - nuclear_norm  + H_p_q + H_p_q + nuclear_norm
...
train_loader = DataLoader(multi_view_dataset, batch_size=args.batch_size, shuffle=True) # normal train and retrain
...
for epoch in range(args.num_epochs):
    vib.train()
    vib = vib_train_with_MCR(train_loader, vib, loss_fn, reconstruction_function, args, epoch, train_type)  # dataloader_total, dataloader_w_o_twin
...
# Get all indices in the original dataset
all_indices = torch.arange(len(original_train_loader.dataset))
# Find the remaining indices by excluding unlearn_indices
remaining_indices = torch.tensor(list(set(all_indices.tolist()) - set(unlearn_indices.tolist())))
# Create a subset dataset with the remaining indices
remaining_dataset = Subset(original_train_loader.dataset, remaining_indices)
# Create a new DataLoader for the remaining dataset
remaining_loader = DataLoader(remaining_dataset, batch_size=original_train_loader.batch_size, shuffle=True)
