...
def unlearning_with_topk(vib, unlearning_loader_with_center, top_k_nearest_loader, loss_fn, reconstruction_function, args):
    ...
    for epoch in range(args.unl_epochs):
        ...
        for x, center_z_new, y, positive_x in unlearning_loader_with_center:
            ...
            loss = 0.1  * (F.mse_loss(logits_z_p, center_z_new) -  F.mse_loss(logits_z, center_z_new) + alpha ) #+ H_p_q # with label
...
poison_samples_size = args.unlearning_size  # 你想要注入后门的样本数量
add_backdoor = 1           # 1 表示添加后门
mode = 'unlearning'        # 仅作为标识，传给 add_trigger_new


triggered_unlearning_dataset, triggered_unlearning_dataset_target = add_trigger_new(
    add_backdoor=add_backdoor,
    dataset=unlearning_dataset,
    poison_samples_size=poison_samples_size,
    mode=mode
)

identity_transform = IdentityTransform()

# 将带后门标签的 triggered_unlearning_dataset 转成 multi_view 的形式
triggered_unlearning_mv = MultiViewCIFAR(
    triggered_unlearning_dataset,
    args.k,
    identity_transform,
    identity_transform
)

combined_multi_view_dataset = ConcatDataset([multi_view_dataset, triggered_unlearning_mv])

combined_loader = DataLoader(
    combined_multi_view_dataset,
    batch_size=args.batch_size,
    shuffle=True
)
...
unlearning_loader = DataLoader(triggered_unlearning_dataset, batch_size=args.batch_size, shuffle=True)
...
for epoch in range(args.num_epochs):
    vib.train()
    vib = vib_train(combined_loader, vib, loss_fn, reconstruction_function, args, epoch, train_type)  # dataloader_total, dataloader_w_o_twin
