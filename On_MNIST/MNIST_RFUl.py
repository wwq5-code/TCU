...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, train_type):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        loss = args.beta * KLD_mean + BCE #args.beta * KLD_mean + BCE + H_p_q - nuclear_norm  # + H_p_q - nuclear_norm  + H_p_q + H_p_q + nuclear_norm
...
unlearn_indices = torch.where(label_1_mask)[0][:10]  # Example: Select the first 10 samples
unlearn_images = [original_train_loader.dataset[i][0].to(device) for i in unlearn_indices]
# Retrieve the labels corresponding to the unlearn_indices
unlearn_labels = [original_train_loader.dataset[i][1] for i in unlearn_indices]

# If you'd like to convert them into a tensor on the same device:
unlearn_labels = torch.tensor(unlearn_labels, device=device)
...
for epoch in range(epochs):
    ...
    # Compute embeddings for the selected samples to unlearn
    unlearn_images_batch = torch.stack(unlearn_images)  # Stack selected images into a batch
    unlearn_labels_batch = unlearn_labels
    z_unlearn, logits_y, x_hat, mu, logvar = vib(unlearn_images_batch, mode='with_reconstruction')  # (B, C* h* w), (B, N, 10)
    # VAE two loss: KLD + MSE

    KLD_element = mu.pow(2).add_(logvar.exp()).mul_(-1).add_(1).add_(logvar).cuda()
    KLD_mean = torch.mean(KLD_element).mul_(-0.5).cuda()

    unlearn_images_batch = unlearn_images_batch.view(unlearn_images_batch.size(0), -1)
    BCE = reconstruction_function(x_hat, unlearn_images_batch)
    ...
    H_p_q = loss_fn(logits_y, unlearn_labels_batch)

    loss = args.beta * KLD_mean -BCE * 0.001
...
vib.eval()
acc = eva_vib(vib, test_loader, args, name='on test dataset after unlearning', epoch=999)
