...
def prepare_unl(erasing_dataset, dataloader_remaining_after_aux, model, loss_fn, args, noise_flag):
    ...
    for epoch in range(1):
        ...
        for (x_e, y_e), (x_r, y_r) in zip(erasing_dataset, dataloader_remaining_after_aux):
            ...
            unlearning_item = args.beta * KLD_mean.item() - args.unlearn_learning_rate * H_p_q.item()
            ...
            backdoor_acc_list.append(acc_back)
            ...
            #     mu_list.append(torch.mean(mu).item())
            #     sigma_list.append(sigma)
            # acc = eva_vib(model, erasing_dataset, args, name='on erased data', epoch=999)
            # if acc < 0.95:
            #     break

        print(f'[{epoch}/{0 + args.num_epochs}:] ' + ', '.join([f'{k} {v:.3f}' for k, v in metrics.items()]))
        x_cpu = x_e.cpu().data
        x_cpu = x_cpu.clamp(0, 1)
        x_cpu = x_cpu.view(x_cpu.size(0), 1, 28, 28)
        grid = torchvision.utils.make_grid(x_cpu, nrow=4)
...
def prepare_centroid_and_positive(vib, remaining_loader, args):
    vib.eval()
    all_embeddings = []
    all_labels = []
...
def triplet_contrastive_unlearning(vib, unlearning_loader, remaining_loader, margin, epochs, args):
    optimizer = torch.optim.Adam(vib.parameters(), lr=args.lr)

    unlearning_loader_with_c_p = prepare_centroid_and_positive(vib, remaining_loader, args)
...
args.beta = 0.0001
args.mcr_rate = 0.001
args.vbu_r = 0.0001 # for vbu
args.mse_rate = 0.1
args.lr = 0.0005
args.distance_rate = 0.01
args.margin = 0.01
args.unlearn_learning_rate = 0.0001
args.ep_distance = 20
